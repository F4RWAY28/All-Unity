Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/efa47f7f1fbfdff46828edb18861bcb3"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void ApplyVoronoi(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float edgeNoise = 0;
				#endif
				edgeNoise *= (0.5 /*_VoronoiNoiseIntensity*/);
				float3 voronoiSpeed = float4(0.8,0.8,0.8,1) * 10;
				#ifdef POI_AUDIOLINK
				if (poiMods.audioLinkAvailable)
				{
				...
				#endif
				if ((2 /*_VoronoiType*/) == 0) // Basic
				{
					voronoi = voronoi2D(position.xy, (2.5 /*_VoronoiScale*/), voronoiSpeed, randomPoint.xy);
					voronoi *= 1.55; // Range adjustment
				}
				if ((2 /*_VoronoiType*/) == 1) // Diff
				{
					voronoi = VoronoiNoiseDiff_Octaves(position, (2.5 /*_VoronoiScale*/), voronoiSpeed, voronoiOctaveNumber, voronoiOctaveScale, voronoiOctaveAttenuation, 1, _Time.x, randomPoint);
				}
				if ((2 /*_VoronoiType*/) == 2) // Fixed Border
				{
					voronoi = voronoi3D(position, (2.5 /*_VoronoiScale*/), voronoiSpeed, randomPoint);
					voronoi *= 1.8; // Range adjustment
				}
				float4 outerColor = float4(0.01061847,0.01061847,0.01061847,1);
				float4 innerColor = float4(0,0,0,1);
				if ((0.0 /*_VoronoiEnableRandomCellColor*/) == 1)
				{
				...
				voronoi = pow(voronoi, (0.4545454 /*_VoronoiPower*/));
				float2 voronoiGradient = float4(0,0.5,0,0).xy + edgeNoise;
				#ifdef POI_AUDIOLINK
				...
				float outerEmissionStrength = (3.0 /*_VoronoiOuterEmissionStrength*/);
